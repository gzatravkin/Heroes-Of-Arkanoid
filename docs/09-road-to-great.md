...
| Enemies | **B+** | All 12 types exist and are tested — but they're spread across only 14 levels, so most appear once. No encounter *design* (combinations, escalation, set-pieces). Deep mechanics/art audit: `docs/10-enemy-audit.md` (3 wrong-art bugs found+fixed). Design-level evaluation + redesign proposals (design language, danger-pays rule, restored decision mechanics): `docs/11-enemy-design-proposals.md`. |
| Game design (core loop) | **B** | Two-resource survival + imbue/active casting + boss phases works and is balanced (P7a). But the four build axes are shallow: 4 relics can't make a synergy web. |
...
| Skills | **C+** | 13 spells with linear leveling. Non-FM classes have 3-spell kits (the originals had 5). No loadout *decisions* — you get everything your class has. Old kit spells (Phoenix, Penetration, Last Day, Magnet) were never ported. |
| Level design | **C** | 14 competent but stamp-symmetric layouts occupying the top half of the board. One win condition (clear/boss). The original's StrechedLevel (descending pressure) and multi-floor modes were never ported. Per-biome identity spec (layout idioms, exclusive pacing modes + objective flavors, atmosphere kits, greyscale test, gen-levels lint rules): `docs/12-biome-identity.md` — this is G3's design source. |
| Meta / retention | **C** | Campaign + 2 fixed rifts + achievements. All characters free from the start (design doc called them *unlockable*); no mastery tracks, no ascension, no seeded/daily runs. Beat the campaign once and you're done. |
| "Shiny" (juice + audio) | **D** | Trails, shake, vignette exist. **Audio is zero** — no impact sounds, no music. A silent breakout game cannot feel great; this is the single loudest gap. |
