...
| Meta / retention | **A− (was C)** | **2026-06-10:** 30-level campaign with forks, seeded generated rifts (3-5 floors, boss finale, biome-keyed rewards, synergy-hinted picks), **ascension tiers 1-5**, and **earned characters** — fresh saves start as the Fire Mage and unlock the roster through boss clears (existing saves untouched). Descoped by design decision: a daily seeded rift would break the deliberate rifts-only-entry rule (docs/04 §10's "yes/no?" answered: no). Remaining to A: per-character mastery tracks. |
| "Shiny" (juice + audio) | **B+ (was D)** | **2026-06-10:** SFX (~35 synthesized event cues) AND per-biome generative music (Hell drone + metallic impacts, Caverns knocks, Witchland detuned chimes, Heaven choral pads — the docs/12 briefs) — all zero-asset, toggle-honoring. Plus the render overhaul: mirrored symmetric paddle, native-aspect bricks with mortar gaps, tiled walls, standing statues, real cracked damage frames, bubbling cauldron strips. Remaining to A: sampled audio (recipes are drop-in replaceable when assets are approved). |
