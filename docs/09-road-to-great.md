...
| Level design | **B (was C)** | **2026-06-09 update:** campaign re-authored to 21 levels against the `docs/12` identity matrix — each biome has its two idioms, an exclusive pacing mode (descend / floors / revive clock / escalation), an exclusive objective flavor (breach / timeLimit / exorcism / survive), atmosphere kits, and 5 machine-enforced lint rules in `gen-levels.mjs`. Remaining to A: 30+ levels, rift matrix inheritance, fork nodes in the campaign graph. |
