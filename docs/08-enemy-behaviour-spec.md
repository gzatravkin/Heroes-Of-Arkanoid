...
**ALL 12 ENEMY TYPES DONE.** Remaining refinements/polish: **He5/He6** altar/vase ally-toggle, **He4** columns, Hell **H3** lava, Caverns **C3** cart, ~~Witch boss magic~~ ✅ (village boss tags every bolt `kind="witchmagic"`; HazardLayer cycles the `WitchMagic1-4` sprites — `BossTests.WitchBoss_TagsHazardsAsWitchMagic` + `enemies.spec` "Witch boss casts magic bolts"), Russian badges (D2, needs art), HUD spell-emoji fallbacks, ~~stretched-art→9-slice (H)~~ ✅ (shared `ui/nineSlice.ts`; every `/100% 100%` button/plate — Settings, Skills, Dungeon(s), Tutorial, Menu dock, Campaign Upgrades + mission plates — now border-image 9-slice. Proof: `nineslice-shots.spec` × 4 scenes).
